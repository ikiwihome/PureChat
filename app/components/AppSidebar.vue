      ...
      <div v-for="group in groupedSessions" :key="group.groupName">
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              <!-- 遍历分组后的会话 -->
              <SidebarMenuItem v-for="session in group.sessions"
:key="session.id"
                :class="{ 'rounded-sm': currentSessionId === session.id }">
                <SidebarMenuButton @click="handleSelectSession(session.id)"
@mouseenter="hoveredSessionId = session.id"
                  @mouseleave="hoveredSessionId = null"
:class="['w-full flex items-center justify-between hover:bg-gray-200/50 dark:hover:bg-gray-200/10',
                    { 'bg-gray-300/50 dark:bg-gray-200/10 rounded-sm': currentSessionId === session.id }]">
                  <img :src="getProviderIcon(session.providerId)"
:alt="session.providerId" 
                  ...
                  <button v-show="hoveredSessionId === session.id"
