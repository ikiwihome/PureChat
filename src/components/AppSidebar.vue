<template>
  <Sidebar v-bind="props" :open="isMobile ? openMobile : open"
    @update:open="(val: boolean) => isMobile ? setOpenMobile(val) : setOpen(val)">
    <SidebarContent>
      <!-- New Chat Button -->
      <div class="sticky bg-sidebar left-0 right-0 top-0 z-20 pt-3.5 dark:bg-gray-750 p-2">
        <div class="pb-0.5 last:pb-0 rounded-lg" tabindex="0">
          <SidebarMenuButton type="button" @click="onCreateNewSession"
            class="group flex h-10 items-center gap-2 rounded-lg px-2 font-medium w-full justify-start text-token-text-primary hover:bg-gray-200/50 dark:hover:bg-gray-200/10">
            <div class="grow overflow-hidden text-ellipsis whitespace-nowrap text-sm">
              创建新会话
            </div>
            <span class="flex items-center">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                class="text-black dark:text-white h-[18px] w-[18px]">
      ...
      <div v-for="group in groupedSessions" :key="group.groupName">
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            <SidebarMenu>
              <!-- 遍历分组后的会话 -->
              <SidebarMenuItem v-for="session in group.sessions" :key="session.id"
                :class="{ 'rounded-sm': currentSessionId === session.id }">
                <SidebarMenuButton @click="handleSelectSession(session.id)" @mouseenter="hoveredSessionId = session.id"
                  @mouseleave="hoveredSessionId = null" :class="['w-full flex items-center justify-between hover:bg-gray-200/50 dark:hover:bg-gray-200/10',
                    { 'bg-gray-300/50 dark:bg-gray-200/10 rounded-sm': currentSessionId === session.id }]">
                  <img :src="getProviderIcon(session.providerId)" :alt="session.providerId" 
                    class="h-4 w-4 dark:filter dark:invert shrink-0 text-gray-500 dark:text-gray-400 mr-2" />
                  <div class="flex-grow overflow-hidden text-ellipsis whitespace-nowrap text-sm">
                    {{ session.title || '新会话' }}
                  </div>
                  <button v-show="hoveredSessionId === session.id" @click.stop="confirmDeleteSession(session.id)"
                    class="ml-2 p-1 rounded-md hover:bg-red-100 dark:hover:bg-red-800 text-red-500 flex transition-opacity">
