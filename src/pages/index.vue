<template>
  <SidebarProvider>
    ...
    <SidebarInset class="flex flex-1 flex-col">
      ...
      <header
        class="w-full fixed top-0 bg-white dark:bg-black z-30 flex justify-between lg:h-16 h-20 shrink-0 items-center gap-2 px-4">
        <div class="flex fixed top-4 sm:gap-2">
          ...
          <Menubar class="lg:mx-5 px-1 border-none">
            <MenubarMenu>
              <MenubarTrigger
                class="w-25 sm:w-40 flex items-center justify-between gap-4 font-bold hover:bg-gray-200/50 dark:hover:bg-gray-200/20">
                <div class="flex space-x-2">
                  <img :src="selectedProvider.icon" :alt="selectedProvider.name"
                    class="h-5 w-5 dark:filter dark:invert hidden sm:block" />
              ...
              <MenubarContent class="border-none">
                <MenubarItem v-for="provider in providers" :key="provider.id" @select="selectProvider(provider)"
                  ...
                  <svg v-if="selectedProvider.id === provider.id"
                    class="h-4 w-4 mr-0 icon-md block text-black dark:text-gray-200" width="24" height="24"
          ...
          <Select v-model="selectedModel">
            <SelectTrigger
              class="pointer-cursor relative flex items-center justify-between rounded-md border hover:bg-gray-200/50 dark:hover:bg-gray-200/20 border-black/10 bg-white py-2 pl-3 pr-3 text-left focus:outline-none focus:ring-0 focus:ring-offset-0 dark:border-gray-700 dark:bg-black sm:text-sm radix-state-open:bg-gray-50 dark:radix-state-open:bg-gray-700">
              <span
                class="flex h-6 items-center gap-1 text-gray-800 dark:text-gray-200 text-xs min-w-[75px] font-normal">
                {{ currentModel.name }}
              </span>
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectItem v-for="model in currentProviderModels" :key="model.id" :value="model.id">
                  <span class="inline-flex w-full">
                    <span
                      class="flex h-6 items-center gap-1 text-gray-800 dark:text-white text-xs min-w-[75px] font-normal">
                      {{ model.name }}
                    </span>
                  </span>
                </SelectItem>
        ...
        <ScrollArea class="h-[calc(100vh-8rem)]">
          <div class="space-y-2 px-2">
            <div v-for="provider in providers" :key="provider.id"
              class="border-b border-gray-200 dark:border-gray-700 py-4 last:border-b-0">
        ...
        <ScrollArea class="h-[calc(100vh-8rem)]">
          <div class="px-4 space-y-6">
            ...
            <div class="space-y-4">
              ...
              <div v-for="provider in providers" :key="provider.id"
                class="space-y-3 border-b border-gray-200 dark:border-gray-700 pb-4 last:border-b-0">
                <!-- 厂家名称和SWITCH开关 -->
                <div class="flex items-center justify-between">
                  ...
                  <Switch v-model="providerSettings[provider.id].useCustomApi"
                    @update:model-value="initProviderSettings(provider.id)" />
                </div>
                ...
                <div v-if="providerSettings[provider.id]?.useCustomApi" class="pl-6 space-y-3">
                  ...
                  <div class="space-y-2">
                    ...
                    <Input v-model="providerSettings[provider.id].apiBaseUrl"
                  ...
                  <div class="space-y-2">
                    ...
                    <Input v-model="providerSettings[provider.id].apiKey" type="password" placeholder="输入您的API密钥"
                      class="w-full text-sm px-2 py-1 border border-gray-300 rounded-md dark:border-gray-600 dark:bg-gray-200/10 dark:text-white" />
                  </div>
                </div>
              </div>
            ...
            <div class="space-y-4">
              ...
              <Slider v-model="temperature" :min="0.0" :max="1.0" :step="0.1"
                class="w-full mt-4" />
            ...
            <div class="space-y-4">
              ...
              <Textarea v-model="systemPrompt" placeholder="你是一个超级智能的AI大模型，擅长准确理解用户意图，回答用户问题" rows="4"
